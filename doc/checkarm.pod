...
longitude cooordinates are compared. The grid square locator is calculated
from the summit list and compared to the value found from local ARM.
The points attributed to the summit is compared to value found from the
...
close to each other. These could be possible doublicates. 
